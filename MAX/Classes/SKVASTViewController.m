...
- (void)info
{
    ...
    if ([self.delegate respondsToSelector:@selector(vastOpenBrowseWithUrl:url:)]) {
        [self.delegate vastOpenBrowseWithUrl:self url:self.clickThrough];
    }
}
...
- (void)setUpTapGestureRecognizer
{
    ...
    /*
    self.swipeUpGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeUp)];
    self.swipeUpGestureRecognizer.delegate = self;
...
- (void)handlePan {
    if (self.panGestureRecognizer.state == UIGestureRecognizerStateBegan || self.panGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        ...
        self.panGestureRecognizer.view.center = CGPointMake(self.panGestureRecognizer.view.center.x, self.panGestureRecognizer.view.center.y + translation.y);
        ...
        if (self.panGestureRecognizer.view.center.y < self.view.window.bounds.size.height / 3) {
            [UIView animateWithDuration:0.3 animations:^{
                self.panGestureRecognizer.view.center = CGPointMake(self.view.window.bounds.size.width / 2, self.view.window.bounds.size.height * -2);
